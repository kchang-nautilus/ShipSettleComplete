...
//		});
    ...
    function NLS_ValidateLine(scriptContext) {
    	...
    	// Retail ONLY
    	if (sublistName === 'item' && stItemType && iChannel === CONSTANT_CHANNEL_RETAIL && currentForm !== RETAIL_Store_FORM) {
...
//    	alert("ValidateLine sublistName=" + sublistName + ", mShipAddr=" + mShipAddr + ", ShipAddr=" + shipAddr + ", isLeasing=" + isLeasing);
    	if (sublistName === 'item' && stItemType){
    		...
    		if (shipAddr && !lib.inArray(arNotSupprtedItemTypes, stItemType)){
...
//			});
    ...
    function DisableLineFields(objRecord)
    {	// Page Init Function		
...
//					objField.isDisable = bDisabled;
    ...
    function EnableLineFields(currentRecord)
		...
		if ((itemType == 'InvtPart') || (itemType == 'Kit')) {				
			if (arrDisabledColumns) {
				...
				for (var i = 0; i < arrDisabledColumns.length; i++) {
					...
					dialog.alert ({
						title : "EnableLineFields",					
						message : "fieldID=" + fieldID + ', parentID=' + parentID + ', currIndex=' + currIndex
					});
					
					if (fieldID) {
						var bDisabled = true;
//							nlapiDisableLineItemField('item', fildid, bDisabled);
//						var objField = currentRecord.getCurrentSublistField({
//						    sublistId: 'item',
//						    fieldId: 'item'
//						});
//						var objField = currentRecord.getSublistField({
...
//						    line: currIndex
//						});
						var objList = currentRecord.getSublist({
							sublistId: 'item'
						});
						var objColumn = objList.getColumn({
							fieldId: 'amount'
						});
						objField.isDisable = bDisabled;
						if (arrEnabledColumns) {
							for (var j = 0; j < arrEnabledColumns.length; j++) {
								...
								// DF-8822 Price Level is disabled after FOB Price level is set
								if (parID == 3) {
						...
						// Enable the column
						// DF-8861 NLS Direct Sales Professional able to modify Location on a sales order
						if (bDisabled === false) {								
//							var objField = currentRecord.getSublistField({
//							    sublistId: 'item',
//							    fieldId: fieldID.toLowerCase(),
...
//						nlapiDisableLineItemField('item', fildid, false);
     ...
     * @returns
     */
    function SetPriceLevel(currentRecord){
    	// Validate Line
//    	var excl_SO_Form = 162;
    	var currentForm = currentRecord.getValue('customform');
    	...
    	if(currentForm !== FORM_INTERCOMPANY) {			
			...
			if (createPO === 'SpecOrd') {
				...
				if (currentPriceLevel && currentPriceLevel != -1 && currentPriceLevel != 51)
				{					
					var fobPriceLevel = lib.SearchFOBPriceLevels(currentPriceLevel);   							
					if (fobPriceLevel && fobPriceLevel !== currentPriceLevel){
						currentRecord.setCurrentSublistValue({
							sublistId: 'item',
							fieldId: 'price',
							value: fobPriceLevel
...
//					dialog.alert ({
//						title : "SetPriceLevel currentForm=" + currentForm,					
//						message : "Special Order: " + createPO + ', Changed to FOB Price Level=' + fobPriceLevel + ', from current Price Level=' + currentPriceLevel
//					});
				}
             }
    	} 
